...
func (p PromotionDetail) Handle(gCtx *gin.Context) (r serializer.Response, err error) {
	...
	var (
		...
		customData serializer.OutgoingCustomPromotionDetail
		newbieData any
	...
	case models.PromotionTypeCustomTemplate:
		customData, err = handleCustomPromotionDetail(gCtx, p)
		if customData.ParentInfo.Id == 0 || err != nil {
			r = serializer.Err(gCtx, p, serializer.CodeGeneralError, "", err)
			return
		}
...
func handleCustomPromotionDetail(c *gin.Context, p PromotionDetail) (outgoingRes serializer.OutgoingCustomPromotionDetail, err error) {
	if p.ID == 99999 { // TODO remove temporary
		return
	}
