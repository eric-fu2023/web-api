...
func (service *AnnouncementsService) Get(c *gin.Context) (r serializer.Response, err error) {
	...
	brand := c.MustGet(`_brand`).(int)
	agent := c.MustGet(`_agent`).(int)
	err = model.DB.Scopes(model.ByBrandAgentAndPlatform(int64(brand), int64(agent), service.Platform.Platform), model.ByTimeRange, model.ByStatus, model.SortDesc).Find(&announcements).Error
	if err != nil {
