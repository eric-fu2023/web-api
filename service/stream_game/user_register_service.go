...
var ErrVendorRegister = errors.New("register user with stream game failed")
var ErrOthers = errors.New("stream game create user failed")

type UserRegister struct{}

func (c *UserRegister) CreateUser(user model.User, currency string) (err error) {
	gpu := ploutos.GameVendorUser{
		GameVendorId:     consts.GameVendor["stream_game"],
		UserId:           user.ID,
