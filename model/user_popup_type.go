...
func GetSpinByPromotionId(spinPromotionId string) (spin ploutos.Spins, err error) {
	err = DB.Model(ploutos.Spins{}).Where("promotion_id = ?", spinPromotionId).Find(&spin).Error

	return
}

func GetSpin(spinId int64) (spin ploutos.Spins, err error) {
	err = DB.Model(ploutos.Spins{}).Where("id = ?", spinId).Find(&spin).Error

	return
}

func GetSpinItemsBySpinId(spinId int64) (spinItems []ploutos.SpinItem, err error) {
	err = DB.Model(ploutos.SpinItem{}).Where("spin_id = ?", spinId).Order(`id DESC`).Find(&spinItems).Error
