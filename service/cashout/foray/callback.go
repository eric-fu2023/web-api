...
func (s *ForayTransferCallback) Handle(c *gin.Context) (err error) {
	...
	if s.IsSucess() {
		...
		if cErr == nil {
			go func() {
				pErr := on_cash_orders.Handle(c, updatedCashOrder, ploutos.TransactionTypeCashOut, on_cash_orders.CashOrderEventTypeClose, on_cash_orders.PaymentGatewayForay, on_cash_orders.RequestModeCallback)
				if pErr != nil {
		...
		_, err = cashout.RevertCashOutOrder(c, s.MerchantOrderNo, util.JSON(s), "refund", ploutos.CashOrderStatusFailed, model.DB)
	}
	if err != nil {
		return
	}
