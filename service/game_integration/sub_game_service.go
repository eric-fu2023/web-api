...
func (service *SubGameService) FeaturedList(c *gin.Context) (serializer.Response, error) {
	...
	tx := model.DB.Debug().Model(ploutos.SubGameBrand{}).Where(fmt.Sprintf("%s.brand_id = %d", ploutos.SubGameBrand{}.TableName(), brandId)).Where(fmt.Sprintf("%s.%s = ?", ploutos.SubGameBrand{}.TableName(), platform), 1).Where("is_featured").Order("sort DESC").Find(&subGames)
	if err := tx.Error; err != nil {
		return serializer.Response{
			Data: []serializer.SubGamesBrandsByGameType{},
		}, err
