...
func (p PNG) TransferFrom(ctx context.Context, tx *gorm.DB, user model.User, _ string, _ string, gameVendorId int64, extra model.Extra) (err error) {
	...
	util.Log().Info("PNG GAME INTEGRATION TRANSFER OUT game_integration_id: %d, user_id: %d, balance: %.4f, status: %d, tx_id: %s", util.IntegrationIdEvo, user.ID, res.Balance, res.Result, res.TransID)
...
func handleFailedTransaction(tx *gorm.DB, user model.User, userBalance float64, TransID string, gameVendorId int64) {
	...
	if err != nil {
		log.Printf("Error fetching transaction details from PNG, err: %v", err)
		return
	}
	...
	if transaction.Result == "Y" {
		...
		// NOT THE AMOUNT FROM INITIAL REQUEST, just to keep all consistent with PNG system
...
func (p PNG) TransferTo(ctx context.Context, tx *gorm.DB, user model.User, sum ploutos.UserSum, currency string, gameCode string, gameVendorId int64, extra model.Extra) (balance int64, err error) {
	...
	util.Log().Info("PNG GAME INTEGRATION TRANSFER IN game_integration_id: %d, user_id: %d, balance: %.4f, status: %d, tx_id: %s", util.IntegrationIdEvo, user.ID, util.MoneyFloat(sum.Balance), res.Result, res.TransID)
